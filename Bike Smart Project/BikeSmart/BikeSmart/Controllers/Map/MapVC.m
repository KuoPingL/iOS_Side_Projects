...
static CGFloat kClusterItemWidth = 40;
...
- (void)viewDidLoad {
    ...
    _isFirstLoad = true;
    [UserDefaults setClusterItemWidth:kClusterItemWidth];
}
...
//        }
        ...
        _icons[iconKey] = [BikeClusterItem getIconWithRect:CGRectMake(0, 0, kClusterItemWidth, kClusterItemWidth) forBikeModel:bikeModel];
    }
...
- (void)renderCluster:(GMSMarker *)marker {
    ...
    if ([currentCluster.items.firstObject isKindOfClass:[BikeClusterItem class]]) {
        ...
        marker.icon = [BikeCluster getClusterImageWithItems:numberOfItems forModel:bikeModel withSize:CGSizeMake(80, 80)];;
    }
...
- (void)notificationReceived:(NSNotification *)notification {
    ...
    } else if ([name isEqualToString:NSUserDefaultsDidChangeNotification]) {
        if ([UserDefaults getClusterItemWidth] != kClusterItemWidth){
            kClusterItemWidth = [UserDefaults getClusterItemWidth];
